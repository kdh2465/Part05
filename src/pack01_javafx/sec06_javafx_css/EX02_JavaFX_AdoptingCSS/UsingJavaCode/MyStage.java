...
public class MyStage extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Label label = new Label("rotation text");
		...
		//@CSS 적용을 위한 id/styleClass 정의 =========================================
		textField1.getStyleClass().add("forward");
		textField2.getStyleClass().add("forward");
		textField3.getStyleClass().add("backward");
		textField4.getStyleClass().add("backward");
		textField5.getStyleClass().add("bidirect");
		textField6.getStyleClass().add("bidirect");
		
		label.setId("label");
		slider.setStyle("-fx-border-color:green;-fx-border-width:3;");
		slider.setId("slider");
